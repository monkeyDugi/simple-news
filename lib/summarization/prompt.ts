...
export function buildUserMessage(items: SummaryInput[]): string {
  ...
  return `다음 기사 목록을 위 규칙대로 요약해 주세요.\n\n${JSON.stringify(payload, null, 2)}`;
}

// ───── 출력 검증 ─────────────────────────────────────────
// guide.md 보다 한도를 살짝 너그럽게 잡아 가끔 발생하는 경계 초과를 흡수.
// (운영하면서 분포 보고 좁힐 예정)
...
export const summaryArraySchema = z.array(summarySchema);

export type SummaryItem = z.infer<typeof summarySchema>;

// ───── 응답 파서 (3단계 폴백) ────────────────────────────
// 모델이 가끔 ```json``` 펜스 / 앞뒤 잡설을 붙이는 경우를 대비.
